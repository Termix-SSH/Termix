 ...
 * are what end up stored in a role. They have to keep matching what automations
...
describe("automations permissions", () => {
  ...
  it("registers exactly the ids roles already hold", () => {
    const ids = (manifest.manifest?.contributes?.permissions ?? []).map(
      (permission) => qualifyPermission("automations", permission.name),
    );
