...
/**
 * Room state that has to be shared between Termix instances: who controls a
 * stage, pending control requests, presence and room events. Redis-backed when
 * REDIS_URL is set, with local state for a single instance.
 */
export class CollabRuntimeStore {
  ...
  async close(): Promise<void> {
    ...
    this.nextConnectAttempt = Number.POSITIVE_INFINITY;
  ...
  private async connect(url: string): Promise<boolean> {
    try {
      ...
      this.log.info("Collaboration Redis runtime connected");
  ...
  private logRedisFailure(operation: string, error: unknown): void {
    this.log.warn(
      `Collaboration Redis runtime unavailable (${operation}), using local state: ${
