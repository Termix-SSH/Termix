...
function RssFeedWidget({
  ...
  useEffect(() => {
    ...
    const fetchFeed = async () => {
      try {
        setLoading(true);
        setError(false);
        const res = await api.get<RssItem[]>("/rss", {
          params: { url: feedUrl, max: maxItems ?? 10 },
        });
  ...
  if (!feedUrl) {
    return (
      <div className="flex flex-col items-center justify-center w-full h-full gap-2 text-muted-foreground/50">
        <Rss size={24} />
        <span className="text-xs">{t("homepage.rssNoFeedUrl")}</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col w-full h-full overflow-hidden">
      <WidgetTitle
        title={widget.title || t("homepage.widgetRssName")}
        icon={<Rss size={11} />}
      />
