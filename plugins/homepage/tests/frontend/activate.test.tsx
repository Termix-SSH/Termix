...
describe(`${manifest.id} activate`, () => {
  ...
  it("registers the homepage tab as a singleton", async () => {
    rendered = await renderWithApp(plugin, { manifest, locales });
    expect(rendered.registered.tabs()).toContain("homepage");
  });

  it("registers both dashboard cards", async () => {
    rendered = await renderWithApp(plugin, { manifest, locales });
    expect(rendered.registered.dashboardCards().sort()).toEqual([
      "homepage_preview",
      "service_links",
    ]);
  });

  it("contributes to the dashboard.secondaryView slot", async () => {
    rendered = await renderWithApp(plugin, { manifest, locales });
    expect(rendered.registered.slot("dashboard.secondaryView")).toEqual([
      "homepage",
    ]);
  });
  ...
  it("removes everything it registered on deactivate", async () => {
    ...
    expect(app.registered.slot("dashboard.secondaryView")).toEqual([]);
