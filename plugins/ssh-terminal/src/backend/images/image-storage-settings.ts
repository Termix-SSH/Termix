...
export interface SettingSource {
  ...
  /** Told about a stored or environment value that was ignored. */
...
export async function resolveTerminalImageStorageSettings(
  ...
  const warnInvalid = (key: string, source: string) =>
    settings.warn?.(
      `Ignoring invalid terminal image storage setting ${key} from ${source}`,
    );
