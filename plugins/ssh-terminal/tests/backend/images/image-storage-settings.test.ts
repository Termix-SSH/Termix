...
describe("terminal image storage settings", () => {
  ...
  describe("resolution precedence", () => {
    ...
    it("reads the plugin's admin settings, and treats unset fields as unset", async () => {
      const { ctx } = createFakeContext({
        settings: {
          imageStorageMode: "local",
          imageLocalDir: "/persisted/images",
        },
      });
      const resolved = await readImageStorageSettings(ctx);
      expect(resolved.mode).toBe("local");
      expect(resolved.localDir).toBe(localDir("/persisted/images"));
      // Nothing stored and no environment: the built-in default.
      expect(resolved.maxCount).toBe(DEFAULT_IMAGE_MAX_COUNT);
    });

    it("keeps TERMINAL_IMAGE_STORAGE_KEYS as the lookup names", () => {
      expect(TERMINAL_IMAGE_STORAGE_KEYS.mode).toBe(
