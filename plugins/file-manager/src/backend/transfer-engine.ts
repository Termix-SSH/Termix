    ...
    cancelWatchdogs.set(
      ...
      setTimeout(() => {
        ...
        if (
          ...
          pluginCtx().log.warn(
            `Force-finalized stuck transfer after cancel: ${transferId}`,
          );
        }
      }, 8000),
    );
...
async function finalizeStreamTransferIfDestAtSize(
  ...
  try {
    ...
    pluginCtx().log.info(
      `Destination file complete, finalizing transfer: ${transferId} (destSize=${destSize}, expectedSize=${expectedSize})`,
    );
...
async function reconnectDedicatedTransferSessions(
  ...
  pluginCtx().log.warn(
    `Reconnecting dedicated transfer SSH sessions: ${ctx.transferId} (parallelWorkers=${parallelWorkers})`,
  );
...
async function resetDedicatedTransferSessions(
  ...
  pluginCtx().log.warn(
    `Resetting transfer SSH sessions (fresh connection): ${ctx.transferId} (attempt=${attempt}, reason=${reason})`,
  );
...
function checkHungTransfers(): void {
  ...
  for (const [transferId, progress] of activeTransfers) {
    ...
    if (
      ...
      pluginCtx().log.info(
        `Transfer progress at 100%, marking complete: ${transferId} (${progress.bytesTransferred}/${progress.totalBytes})`,
      );
      finalizeTransfer(transferId, {
        status: "success",
        phase: "transferring",
    ...
    pluginCtx().log.error(
      `Force-finalizing unresponsive transfer: ${transferId} (lastActivityAt=${lastActivity}, bytesTransferred=${progress.bytesTransferred})`,
    );
...
async function runFastSftpCopySegmentedSequential(
  ...
  while (offset < fileSize) {
    ...
    while (true) {
      pluginCtx().log.info(
        `Transfer segment started: ${transferId} segment ${segmentIndex}/${segmentCount} (offset=${offset}, len=${segLen}, attempt=${attempts + 1})`,
      );
      ...
      try {
        ...
        pluginCtx().log.info(
          `Transfer segment completed: ${transferId} segment ${segmentIndex}/${segmentCount} (offset=${offset}, len=${segLen})`,
        );
        ...
        pluginCtx().log.error(
          `Transfer segment failed: ${transferId} segment ${segmentIndex} (offset=${offset}, len=${segLen}, attempt=${attempts + 1}/${SFTP_SEQUENTIAL_SEGMENT_MAX_ATTEMPTS}, recoverable=${recoverable})`,
          err as Error,
        );
        ...
        if (resumeOffset > offset) {
          pluginCtx().log.info(
            `Resuming transfer from destination size: ${transferId} (previousOffset=${offset}, resumeOffset=${resumeOffset}, fileSize=${fileSize})`,
          );
          offset = resumeOffset;
...
async function runFastSftpCopySegmentedParallel(
  ...
  pluginCtx().log.info(
    `Starting parallel segment transfer: ${transferId} (parallel=${parallel}, segmentJobs=${jobs.length}, fileSize=${fileSize}, destResumeSize=${destResumeSize})`,
  );
  ...
  const resetParallelLane = async (
    ...
    pluginCtx().log.warn(
      `Resetting parallel transfer lane (fresh SSH session): ${reconnect.transferId} lane ${workerId} (attempt=${attempt}, reason=${reason})`,
    );
  ...
  const runSegmentJob = async (
    ...
    while (true) {
      ...
      pluginCtx().log.info(
        `Transfer segment started: ${transferId} segment ${job.segmentIndex} lane ${workerId} (offset=${job.offset}, len=${job.length}, attempt=${attempts + 1})`,
      );
      ...
      try {
        ...
        pluginCtx().log.info(
          `Transfer segment completed: ${transferId} segment ${job.segmentIndex} lane ${workerId} (offset=${job.offset}, len=${job.length})`,
        );
        ...
        pluginCtx().log.error(
          `Transfer segment failed: ${transferId} segment ${job.segmentIndex} lane ${workerId} (offset=${job.offset}, len=${job.length}, attempt=${attempts + 1}/${SFTP_PARALLEL_SEGMENT_MAX_ATTEMPTS}, recoverable=${recoverable})`,
          err as Error,
        );
    ...
    const runCopyWithRetry = async (): Promise<void> => {
      ...
      while (true) {
        ...
        if (resumeOffset > 0) {
          ...
          if (attempts === 0) {
            pluginCtx().log.info(
              `Resuming transfer from destination size: ${transferId} (resumeOffset=${resumeOffset}, fileSize=${fileSize})`,
            );
          }
        }
        ...
        try {
          ...
          pluginCtx().log.error(
            `Transfer copy attempt failed: ${transferId} (attempt=${attempts + 1}/${maxCopyAttempts}, recoverable=${recoverable}${isParallelCopy ? `, parallelLanes=${parallelLanes}` : ""})`,
            err as Error,
          );
          ...
          if (isParallelCopy) {
            ...
            pluginCtx().log.warn(
              `Restarting parallel copy after segment exhaustion: ${transferId} (attempt=${attempts}, delayMs=${delayMs})`,
            );
            await new Promise((resolve) => setTimeout(resolve, delayMs));
          } else {
...
async function tryAdaptiveDirectTransfer(
  ...
  try {
    ...
    pluginCtx().log.warn(
      `Direct transfer failed, falling back to relay: ${transferId} (${getErrorMessage(error, "Direct transfer failed")})`,
    );
    if (reconnectMeta.profileKey) {
      recordDirectRouteOutcome(
        reconnectMeta.profileKey,
...
async function runTransfer(
  ...
  try {
    ...
    if (!useArchive) {
      ...
      pluginCtx().log.info(
        `Resolved archive transfer method: ${transferId} (methodPreference=${methodPreference}, archiveMethod=${archiveMethod}, fileCount=${scanSummary.fileCount}, totalBytes=${scanSummary.totalBytes}, incompressibleRatio=${scanSummary.incompressibleRatio})`,
      );
    ...
    pluginCtx().log.info(
      `Host transfer completed: ${transferId} (sourcePaths=${sourcePaths.join(", ")}, destPath=${destPath}, move=${move}, totalMs=${elapsedMs(runStart)})`,
    );
    updateTransfer(transferId, {
      moveRequested: move,
      timings: {
    ...
    if (err instanceof TransferCancelledError) {
      ...
      pluginCtx().log.info(
        `Host transfer cancelled: ${transferId} (sourcePaths=${sourcePaths.join(", ")}, destPath=${destPath})`,
      );
      return;
    }
    ...
    if (
      ...
      try {
        ...
        if (finalized) {
          pluginCtx().log.info(
            `Host transfer succeeded after destination verify: ${transferId} (sourcePaths=${sourcePaths.join(", ")}, destPath=${destPath})`,
          );
          return;
        }
      } catch {
    ...
    pluginCtx().log.error(
      `${err instanceof TransferStalledError ? "Host transfer stalled" : "Host transfer failed"}: ${transferId} (sourcePaths=${sourcePaths.join(", ")}, destPath=${destPath})`,
      err as Error,
    );
  } finally {
