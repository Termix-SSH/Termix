...
export function registerFileActionRoutes(
  ...
  /**
   * @openapi
   * /plugin-api/file-manager/copyItem:
   *   post:
   ...
   */
  app.post("/copyItem", async (req, res) => {
  ...
  /**
   * @openapi
   * /plugin-api/file-manager/executeFile:
   *   post:
   ...
   */
  app.post("/executeFile", async (req, res) => {
    ...
    if (!sshConn || !sshConn.isConnected) {
      ctx.log.error(
        `SSH connection not found or not connected for executeFile: ${sessionId} (hasConnection=${!!sshConn}, isConnected=${sshConn?.isConnected})`,
      );
    ...
    execChannel(sshConn, checkCommand, (checkErr, checkStream) => {
      if (checkErr) {
        ctx.log.error("SSH executeFile check error:", checkErr);
        return res
          .status(500)
      ...
      checkStream.on("close", () => {
        ...
        execChannel(sshConn, executeCommand, (err, stream) => {
          if (err) {
            ctx.log.error("SSH executeFile error:", err);
          ...
          stream.on("close", (code) => {
            ...
            ctx.log.info(
              `File execution completed: ${sessionId} (${filePath}, exitCode=${actualExitCode}, outputLength=${cleanOutput.length}, errorLength=${errorOutput.length})`,
            );
          ...
          stream.on("error", (streamErr) => {
            ctx.log.error("SSH executeFile stream error:", streamErr);
            if (!res.headersSent) {
  ...
  /**
   * @openapi
   * /plugin-api/file-manager/changePermissions:
   *   post:
  ...
  app.post("/changePermissions", async (req, res) => {
    ...
    if (!sshConn || !sshConn.isConnected) {
      ctx.log.error(
        `SSH connection not found or not connected for changePermissions: ${sessionId} (hasConnection=${!!sshConn}, isConnected=${sshConn?.isConnected})`,
      );
      return res.status(400).json({ error: "SSH connection not available" });
    }
    ...
    ctx.log.info(
      `Changing file permissions: ${sessionId} (${path}, permissions=${octalPerms})`,
    );

    const commandTimeout = setTimeout(() => {
      if (!res.headersSent) {
        ctx.log.error(
          `changePermissions command timeout: ${sessionId} (${path}, permissions=${octalPerms})`,
        );
        res.status(408).json({
    ...
    execChannel(sshConn, command, (err, stream) => {
      if (err) {
        clearTimeout(commandTimeout);
        ctx.log.error(
          `SSH changePermissions exec error: ${sessionId} (${path}, permissions=${octalPerms})`,
          err,
        );
        if (!res.headersSent) {
          return res
      ...
      stream.on("close", (code) => {
        ...
        if (outputData.includes("SUCCESS")) {
          ctx.log.info(
            `File permissions changed successfully: ${sessionId} (${path}, permissions=${octalPerms})`,
          );
        ...
        if (code !== 0) {
          ctx.log.error(
            `chmod command failed: ${sessionId} (${path}, permissions=${octalPerms}, exitCode=${code}, error=${errorOutput})`,
          );
          if (!res.headersSent) {
            return res.status(500).json({
      ...
      stream.on("error", (streamErr) => {
        clearTimeout(commandTimeout);
        ctx.log.error(
          `SSH changePermissions stream error: ${sessionId} (${path}, permissions=${octalPerms})`,
          streamErr,
        );
        if (!res.headersSent) {
          res
