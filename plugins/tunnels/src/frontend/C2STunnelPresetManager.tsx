...
import { clientTunnelName } from "../shared/tunnel-naming";
import {
  getTunnelModeDescription,
  getTunnelPortLabels,
  getTunnelTypeForMode,
} from "./tunnel-form-utils";
import {
  getTunnelMode,
  normalizeClientTunnel,
  stripClientTunnelDiagnostics,
  type ClientTunnel,
} from "./c2s-tunnel-config-utils";
import {
  apiErrorMessage,
...
export function C2STunnelPresetManager(): React.ReactElement {
  ...
  const [savedLocalConfig, setSavedLocalConfig] = React.useState<
    ClientTunnel[]
  >([]);
  const { hosts: pluginHosts, loaded: hostsLoaded } = useHosts();
  const hosts = React.useMemo<SshHostOption[]>(
    () =>
      pluginHosts.map((host) => ({
        id: Number(host.id),
        name: host.name,
        syncId: host.syncId as string | null | undefined,
        connectionType: host.connectionType as string | null | undefined,
      })),
    [pluginHosts],
  );
  ...
  const handleC2SRemoteAuthError = React.useCallback(
    (result: { code?: string; error?: string }) => {
      ...
      toast.warning(t("tunnels.remoteSessionExpired"));
      return true;
    },
    [t],
  );
  ...
  const getTunnelName = clientTunnelName;
  ...
  const refreshLocalConfig = React.useCallback(async () => {
    ...
    const [config, defaultName] = await Promise.all([
  ...
  React.useEffect(() => {
    if (!isElectron || !hostsLoaded) return;
    Promise.all([refreshLocalConfig(), refreshPresets()]).catch(() => {
      setPresets([]);
    });
  }, [isElectron, hostsLoaded, refreshLocalConfig, refreshPresets]);
  ...
  const handleRenamePreset = async () => {
    ...
    try {
      ...
      toast.error(apiErrorMessage(error, t("profile.c2sPresetRenameError")));
    }
  };
  ...
  const handleDeletePreset = async () => {
    ...
    try {
      ...
      toast.error(apiErrorMessage(error, t("profile.c2sPresetDeleteError")));
    }
  };
