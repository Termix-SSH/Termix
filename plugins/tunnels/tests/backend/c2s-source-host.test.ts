...
import { resolveC2SSourceHostId } from "../../src/backend/c2s-relay.js";

describe("resolveC2SSourceHostId", () => {
  it("uses the row matching the stable sync id", async () => {
  ...
  it("keeps a local id when no sync id is available", async () => {
    await expect(
