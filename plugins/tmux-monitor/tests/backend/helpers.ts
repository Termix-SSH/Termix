...
export async function startServer(
  options: {
    ...
    /** hostId -> enableTmuxMonitor, set through ctx.settings before activate. */
    enabledHosts?: number[];
    sshClient?: unknown;
  } = {},
): Promise<TestServer> {
  ...
  for (const h of options.hosts ?? [host()]) {
    db.sqlite.prepare("INSERT INTO ssh_data (id) VALUES (?)").run(h.id);
  }
  ...
  const mock = createMockCtx({
    ...
    permissions: options.permissions ?? ["tmux-monitor.use"],
    hosts: options.hosts ?? [host()],
    sshClient: options.sshClient,
  });

  for (const hostId of options.enabledHosts ?? [7]) {
    await mock.ctx.settings.setHost(hostId, "enableTmuxMonitor", true);
  }
