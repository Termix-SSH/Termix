    ...
    <div className="flex-1 min-h-0 overflow-y-auto scrollbar-none">
      <div className="flex flex-col gap-3 p-3">
        {!identity ? (
          ...
          <>
            <IdentityCard api={api} identity={identity} onChanged={changed} />
            <AddKey
              api={api}
...
function AddKey({
  ...
  async function addManual() {
    ...
    try {
      ...
      toast.error(errorMessage(e, t("termixId.addKeyFailed")));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function generate() {
    ...
    try {
      ...
      toast.error(errorMessage(e, t("termixId.generateFailed")));
    } finally {
      setGenerating(false);
    }
  }
  ...
  async function importCredential(id: number) {
    setSubmitting(true);
    try {
      await api.addKey({ credentialId: id });
      toast.success(t("termixId.imported"));
      onAdded();
    } catch (e) {
      toast.error(errorMessage(e, t("termixId.importFailed")));
    } finally {
      setSubmitting(false);
    }
  }
...
function KeyList({
  ...
  async function issueCert(k: TermixIdentityKey) {
    setIssuingId(k.id);
    try {
      const res = await api.issueCertificate(k.id, {
                ...
                {canCert && (
                  <>
                    <Input
                      ...
                      aria-label={t("termixId.principalsLabel")}
