...
describe("parseCertValidBefore", () => {
  ...
  beforeAll(() => {
    ...
    try {
      execFileSync("ssh-keygen", ["-t", "ed25519", "-f", `${dir}/ca`, "-N", "", "-q"]);
      execFileSync("ssh-keygen", ["-t", "ed25519", "-f", `${dir}/user`, "-N", "", "-q"]);
...
describe("Vault HTTP flow (mocked fetch)", () => {
  ...
  it("startVaultOidc posts auth_url and extracts state", async () => {
    ...
    const result = await startVaultOidc(profile, "https://termix/vault/oidc/callback");
  ...
  it("signWithVault posts the public key and returns signed_key", async () => {
    const calls = mockFetch(() => ({
      body: { data: { signed_key: "ssh-ed25519-cert-v01@openssh.com AAAAcert" } },
    }));
