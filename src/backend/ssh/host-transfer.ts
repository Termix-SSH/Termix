...
export type { TransferMethodPreference, TransferScanSummary } from "./transfer-routing.js";
  ...
  constructor(byteOffset?: number, segmentIndex?: number) {
    const pos =
      byteOffset !== undefined
        ...
        : "";
    const seg =
      segmentIndex !== undefined ? ` (segment ${segmentIndex})` : "";
...
export function requestTransferCancel(
  ...
  const progress = activeTransfers.get(transferId);
  if (!progress || progress.userId !== userId || progress.status !== "running") {
    return false;
  }
    ...
    cancelWatchdogs.set(
      ...
      setTimeout(() => {
        ...
        if (
          ...
          void forceAbortActiveXfer(transferId);
          finalizeTransfer(transferId, cancelledProgressPatch(current, {
            status: "cancelled",
...
function buildStreamTransferTimings(
  ...
  const prepare =
    prepareDestMs ?? progress?.timings?.prepareDestMs ?? 0;
...
async function resetDedicatedTransferSessions(
  ...
  const { sourceSession, destSession } = await reconnectDedicatedTransferSessions(
    ctx,
    0,
  );
...
function updateTransfer(transferId: string, patch: Partial<TransferProgress>): void {
  const current = activeTransfers.get(transferId);
  if (current) {
...
function checkHungTransfers(): void {
  ...
  for (const [transferId, progress] of activeTransfers) {
    ...
    const lastActivity =
      progress.lastActivityAt ?? progress.startedAt ?? now;
    ...
    finalizeTransfer(transferId, failedProgressPatch(progress, {
      status: "error",
      phase: progress.phase === "reconnecting" ? "transferring" : progress.phase,
...
function noteHopStart(clock: HopWallClock, t: number = performance.now()): void {
...
function buildTransferHopTimings(
  ...
  const destRate = computeTransferMbPerSec(
    stats.bytes,
    stats.destWriteSpanMs,
  );
...
function buildSegmentCopyJobs(
  ...
  for (let offset = initialOffset; offset < fileSize; offset += SFTP_XFER_SEGMENT_SIZE) {
    ...
    const start =
      destResumeSize > offset ? destResumeSize : offset;
    jobs.push({
      offset: start,
...
function closeAllTransferSessions(
  ...
  for (let i = 0; i < parallelWorkers; i++) {
    deps.closeDedicatedTransferSession(
      `${ctx.dedicatedSourceSessionId}:p${i}`,
    );
    deps.closeDedicatedTransferSession(
      `${ctx.dedicatedDestSessionId}:p${i}`,
    );
  }
}
  ...
  while (offset < fileSize) {
    ...
    while (true) {
      ...
      try {
        ...
        try {
          resumeOffset = await probeDestResumeOffset(destSftp, destPath, fileSize);
        } catch {
...
async function pipelinedSftpFile(
  ...
  try {
    ...
    const runCopyWithRetry = async (): Promise<void> => {
      ...
      const parallelLanes = clampParallelSegmentCount(options.parallelSegmentCount);
      ...
      while (true) {
        ...
        try {
          ...
          if (isParallelCopy) {
            ...
            fileLogger.warn("Restarting parallel copy after segment exhaustion", {
...
async function transferFileData(
  ...
  if (isLocalSshEndpoint(destSession.ip)) {
    return pipelinedSftpToLocalFile(sourceSftp, sourcePath, destPath, pipeOptions);
  }
...
async function transferSingleFile(
  ...
  return finalizeTransfer(transferId, {
    ...
    sourceDeleted: move,
    moveRequested: move,
    timings: buildStreamTransferTimings(
      transferId,
      stats.size,
      prepareDestMs,
    ),
...
async function transferViaTar(
  ...
  if (compressResult.code !== 0) {
    throw new Error(compressResult.stderr || "Failed to compress on source host");
  }
  ...
  let bytesTransferred = 0;
  const transferStart = Date.now();
  let xferStats = createEmptyXferStats();
  const syncProgress = (absoluteOffset: number) => {
    bytesTransferred = absoluteOffset;
    updateTransfer(transferId, { bytesTransferred: absoluteOffset });
  };
  ...
  if (extractResult.code !== 0) {
    await cleanupDestItems(deps, destSession, destPath, basenames);
    throw new Error(extractResult.stderr || "Failed to extract on destination host");
  }
...
async function transferViaItemSftp(
  ...
  for (const sourcePath of sourcePaths) {
    const items = await collectFileWorkItems(
      sourceSftp,
      sourcePath,
      destPath,
    );
  ...
  const status: TransferStatus =
    failedPaths.length > 0 ? "partial" : "success";
...
async function runTransfer(
  ...
  const {
    ...
    parallelSegmentCount: requestParallelSegments = DEFAULT_PARALLEL_SEGMENT_COUNT,
  } = request;
  ...
  try {
    ...
    if (!useArchive) {
      ...
      const destHasTar =
        destPlatform === "unix" &&
        (await checkTarAvailable(deps, destSession));
    ...
    finalizeTransfer(transferId, failedProgressPatch(current, {
      status: "error",
      phase: current?.phase ?? "transferring",
      message,
      method: current?.method,
...
export function startHostTransfer(
  ...
  activeTransfers.set(transferId, {
    ...
    parallelSegmentCount: clampParallelSegmentCount(request.parallelSegmentCount),
    requestSnapshot: {
      sourceSessionId: request.sourceSessionId,
      sourcePaths: request.sourcePaths,
      destSessionId: request.destSessionId,
