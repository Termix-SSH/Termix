...
interface SSHSession {
  ...
  hostId?: number;
  username?: string;
  /** Ephemeral SSH session opened solely for a host-to-host transfer. */
...
async function startDedicatedTransferConnect(
  ...
  if (proxyConfig) {
    const proxySocket = await createSocks5Connection(host.ip, host.port, proxyConfig);
    if (proxySocket) {
...
async function openDedicatedTransferSession(
  ...
  if (
    !options?.allowBrowseDisconnected &&
    !browseSession?.isConnected
  ) {
  ...
  const config = await buildDedicatedTransferConnectConfig(host, userId, client);
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    void startDedicatedTransferConnect(client, config, host, userId).catch(fail);
  });
...
app.get("/ssh/file_manager/ssh/transferStatus/:transferId", async (req, res) => {
  const userId = (req as unknown as AuthenticatedRequest).userId;
...
app.post("/ssh/file_manager/ssh/transferCleanup/:transferId", async (req, res) => {
  const userId = (req as unknown as AuthenticatedRequest).userId;
  const transferId = req.params.transferId;
...
app.post("/ssh/file_manager/ssh/transferRetry/:transferId", (req, res) => {
  ...
  if (!retried) {
    return res.status(404).json({ error: "Transfer not found or not retryable" });
  }
