...
import type { SettingsComponentProps } from "../settings-components";
...
export function TailscaleDevicesStatus({ running }: SettingsComponentProps) {
  ...
  const label = (key: string) => t(pluginKey("tailscale", key));
    ...
    <div className="flex items-center justify-between gap-3 py-3">
      <div className="flex flex-col gap-0.5 min-w-0 flex-1">
        ...
        <span className="text-xs text-muted-foreground leading-snug">
          ...
          {status.kind === "ok" &&
            t(pluginKey("tailscale", "settings.devices.count"), {
              count: status.count,
            })}
        </span>
      </div>
