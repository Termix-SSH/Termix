...
export function TransferToHostDialog({
  ...
  const destPathDebounceRef = useRef<ReturnType<typeof setTimeout> | null>(null);
      ...
      await Promise.all(
        candidates.map(async (host) => {
          try {
            ...
            initialStates[host.id] = status?.connected ? "ready" : "disconnected";
          } catch {
  ...
  const loadShortcutsForHost = useCallback(async (hostId: number) => {
    try {
      ...
      setShortcuts(
        (Array.isArray(data) ? data : []) as FolderShortcutEntry[],
      );
    } catch {
      setShortcuts([]);
    }
  ...
  useEffect(() => {
    ...
    const path =
      pendingBrowsePathRef.current ?? (destPath.trim() || "/");
    pendingBrowsePathRef.current = null;
  ...
  const handleDestPathKeyDown = (
    ...
    if (event.key !== "Tab" || event.shiftKey || !selectedHost || !isHostReady) {
      return;
    }
